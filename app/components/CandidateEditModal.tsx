...
export default function CandidateEditModal({
  ...
  const namedColors = ["Red", "Blue", "Cyan", "Yellow", "Fuchsia"];
  ...
  useEffect(() => {
    if (candidate) {
      setFormData({
        ...
        fr_meet_link: candidate.fr_meet_link || "",
        be_meet_link: candidate.be_meet_link || "",
        company: candidate.company || "",
        color_palette: candidate.color_palette || "",
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        (Object.keys(formData) as Array<keyof CandidatePatchData>).forEach(
          (key) => {
            if (formData[key] !== originalData?.[key]) {
              (diff as Record<string, string | number | undefined>)[key] =
                formData[key];
            }
          }
        );
      ...
      const res = await axios.patch(
        `/api/addStudent/${formData.user_id}`,
        diff
      );
        ...
        <div className="bg-white rounded-xl shadow-xl w-full max-w-5xl max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              <div className="flex gap-2">
                <div>
                  ...
                  <PlacedDropDown
                    ...
                    onChange={(val) =>
                      handleInputChange("placement_status", val)
                    }
                  />
                </div>
              ...
              <div className="flex gap-4">
                <div className="space-y-2">
                  ...
                  <DropdownMenuRadioGroupDemo
                    ...
                    onChange={(val) =>
                      handleInputChange("interview_status", val)
                    }
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    onChange={(val) =>
                      handleInputChange("color_palette", val.target.value)
                    }
                    className={`border p-2 rounded-sm bg-${color.toLocaleLowerCase()}-500 `}
                  >
                    {namedColors.map((val) => (
                      <option key={val} value={val} className="bg-none">
                        {val}
                      </option>
                    ))}
                  </select>
            ...
            <div>
              ...
              <input
                type="url"
                value={formData.fr_meet_link}
                onChange={(e) =>
                  handleInputChange("fr_meet_link", e.target.value)
                }
                className="mt-1 w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
              />
            </div>
            <div>
              ...
              <input
                type="url"
                value={formData.be_meet_link}
                onChange={(e) =>
                  handleInputChange("be_meet_link", e.target.value)
                }
                className="mt-1 w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
              />
            </div>
